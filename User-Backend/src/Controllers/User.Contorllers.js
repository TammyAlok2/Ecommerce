...
const signUp = asyncHandler(async(req,res,next)=>{
    ...
    // 5) Create the user 
    // 6) Send the response 
    const {username,email,passward,role}= req.body;
    console.log(username,email,passward,role)

    // checking the blank response 
    if([username,email,passward].some((value)=>value.trim() ==='')){
...
const addProductToCart = asyncHandler(async(req,res)=>{
    const {productId,quantiy} = req.body;
    const product = await Shop.Products.findById(productId)
...
const addedProducted = await user.items.push({
    product: productId,
    quantity,
  });


  user.total += product.price * quantity;
  await user.save();
})



export {signUp,signIn,userInfo,logout,forgotPassward,resetPassward,updatePassward,updateUser,addProductToCart}